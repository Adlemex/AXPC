...
window.adlemxBrowserLang = window.browserLang;
...
function t754__showMore(recid) {
    ...
    showMoreButton.addEventListener('click', function () {
        ...
        if (!document.querySelector('.t-records').getAttribute('data-adlemx-mode')) {
            if (window.lazy === 'y' || document.getElementById('allrecords').getAttribute('data-adlemx-lazy') === 'yes') {
...
function t754__alignButtons_init(recid) {
    ...
    try {
        t754__alignButtons(recid);
        if (document.querySelector('.t-records').getAttribute('data-adlemx-mode') === 'edit') {
...
function t754__hoverZoom_init(recid) {
    ...
    try {
        ...
        var hoverScript = 'lib/adlemx-hover-zoom-1.0.min.js';
...
function t754__updateLazyLoad(recid) {
    ...
    if (!tRecords) return;
    var currentMode = tRecords.getAttribute('data-adlemx-mode');
...
function t754_initPopup(recid) {
    ...
    Array.prototype.forEach.call(linksProd, function (link) {
        ...
        function showPopupHandler(event) {
            ...
            var analitics = link.getAttribute('data-track-popup');
            if (analitics && productFull && window.Adlemx) {
                ...
                if (productName) {
                    ...
                    Adlemx.sendEventToStatistics(analitics, virtTitle)
                }
            }
    ...
    if (popup) {
        ...
        var currentMode = allRecords.getAttribute('data-adlemx-mode');
        var isPublishedPage = currentMode !== 'edit' && currentMode !== 'preview';
        if (isPublishedPage && (window.lazy === 'y' || allRecords.getAttribute('data-adlemx-lazy') === 'yes')) {
...
function t754_copyTypography(recid) {
    var rec = document.getElementById('rec' + recid);
    if (!rec) return;
    var productTitle = rec.querySelector('.t754__title');
    var productDescription = rec.querySelector('.t754__descr');
    if (!productTitle || !productDescription) {
        return
    }
    var titleStyle = productTitle.getAttribute('style');
    var descrStyle = productDescription.getAttribute('style');
    var popupTitles = rec.querySelectorAll('.t-popup .t754__title');
    Array.prototype.forEach.call(popupTitles, function (title) {
        title.setAttribute('style', t754_removeSizeStyles(titleStyle))
    });
    var popupDescription = rec.querySelectorAll('.t-popup .t754__descr, .t-popup .t754__text');
    Array.prototype.forEach.call(popupDescription, function (description) {
        description.setAttribute('style', t754_removeSizeStyles(descrStyle))
    })
}
